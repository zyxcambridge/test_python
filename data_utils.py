...
path="/media/root/dataset/deepblue_small_blue/xiaolan"
model="point"
    ...
    with open(name, "w") as xml:
        ...
        for obj in objs:
            ...
            if obj[0] == "cube":
                ...
                cls = "obj"
				# cls = "obj"
                xml.write(fmt % (cls, pmin[0], pmin[1], pmax[0], pmax[1]))
...
if __name__ == "__main__":
	...
	for x in range(0,len(imgs)):
		...
		for obj in objs:
			...
			currentClass = 0
			name = os.path.splitext(imgs[currentImg])[0] + '.xml'
			saveToXml(name, objs, currentClass, 800, 600)
			zyx_num = zyx_num + 1
			print("----------------end one ---------------- ",zyx_num)



	print("end transform number : ",zyx_num)
